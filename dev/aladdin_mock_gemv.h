...
void gemv_mock(float *inputs, float *weights, float *outputs, MetaData meta_data) {
    float *tmp = inputs;
    inputs = weights;
    weights = tmp;

    int tmp1 = meta_data.m;
    meta_data.m = meta_data.n;
    meta_data.n = meta_data.k;
    meta_data.k = tmp1;
    ...
    int padded_input_dims[4] = { 1, 1, MAX_DIM, 1 };
    int padded_weight_dims[4] = { 1, MAX_DIM, MAX_DIM, 1 };
    int padded_output_dims[4] = { 1, 1, MAX_DIM, 1 };
    ...
    for (int i = 0; i < input_dims[2]; i++) {
        padded_inputs[i] = fp16(inputs[i]);
    }

    // Copy weight data with proper padding and transpose
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 4; j++) {
            padded_weights[j * padded_weight_dims[1] + i] = fp16(weights[i * 4 + j]);
        }
    }
